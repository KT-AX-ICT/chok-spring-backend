...
| 변수 | 기본 예시 | 설명 |
| --- | --- | --- |
| `SPRING_PROFILES_ACTIVE` | `dev` | profile 선택. `dev`=H2 in-memory, `local`=H2 파일모드(영속), `prod`=MySQL. 미지정 시 `dev` |
...
| `ANALYSIS_RUN_ON_STARTUP` | `false` | 시작 시 1회 자동 분석(명시 opt-in) |
| `ANALYSIS_CHUNK_SIZE` | `20` | FastAPI batch 묶음 크기 |
| `ANALYSIS_DEFAULT_LIMIT` | `200` | 스케줄러 1주기 처리 상한 |
| `ANALYSIS_STARTUP_LIMIT` | `500` | 시작 1회 처리 상한 |
...
### dev profile 실행 (기본 · H2 in-memory)

profile을 지정하지 않으면 `dev`로 실행됩니다.

```powershell
.\gradlew.bat bootRun
```

dev profile은 in-memory H2 DB를 사용하며 재시작 시 데이터가 초기화됩니다.
...
### local profile 실행 (H2 파일모드 · 영속)

분석 잡(미분석 FATAL → FastAPI 분석 → 저장) 흐름을 재시작 후에도 유지하며 시연하려면 `local`을 사용합니다.
...
- 분석 스케줄러(5분 주기)는 `local`/`prod`에서만 동작합니다. 시작 시 1회 자동 분석은 `ANALYSIS_RUN_ON_STARTUP=true`로 opt-in 합니다.
