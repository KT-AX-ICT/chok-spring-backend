...
import com.sesac.chok.domain.pattern.entity.PatternCluster;
import com.sesac.chok.domain.pattern.repository.PatternClusterRepository;
...
@SpringBootTest
@Transactional
class PatternClusterPersistenceTest {

    @Autowired
    private PatternClusterRepository patternClusterRepository;

    @Test
    void persistsRoundTrip() {
        PatternCluster saved = patternClusterRepository.save(PatternCluster.builder()
                .patternName("Data TLB Error")
                .description("커널 데이터 TLB 오류 군집")
                .eventTemplate("data TLB error interrupt")
                .importance(90)
                .build());
        patternClusterRepository.flush();

        PatternCluster reloaded = patternClusterRepository.findById(saved.getId()).orElseThrow();
